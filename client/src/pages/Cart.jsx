...
const Cart = () => {
    ...
    const handleRazorpayPayment = async () => {
        try {
            // Check if user has _id, if not fetch fresh user data
            if (!user?._id) {
    ...
    const placeOrder = async ()=>{
        try {
            ...
            if(paymentOption === "COD"){
                ...
                // Validate minimum amount for Stripe (₹50 to meet 50 cents USD requirement)
                const totalAmount = getCartAmount() + getCartAmount() * 2 / 100;
                if(totalAmount < 50){
                    return toast.error("Minimum order amount for Stripe payment is ₹50")
                }
